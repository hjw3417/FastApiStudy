...
## 19. 파이썬 자료구조 — list, tuple, dict, set (자바 비교)

### 4종 기본 컬렉션 (괄호로 구분)

| 기호 | 자료구조 | 예 | 순서 | 변경 | 중복 |
|---|---|---|---|---|---|
| `[]` | **리스트** list | `[1, 2, 3]` | O | O(가변) | O |
| `()` | **튜플** tuple | `(1, 2, 3)` | O | **X(불변)** | O |
| `{key: val}` | **딕셔너리** dict | `{"a": 1}` | O(3.7+) | O | key 유일 |
| `{val}` | **집합** set | `{1, 2, 3}` | X | O | **X(중복제거)** |
...
set()          # 빈 set은 이렇게 만듦
```
`{}`는 **내용으로 구분**: 콜론 있으면(`{k: v}`) dict, 값만(`{v}`) set. **빈 `{}`는 dict.**

### ⚠️ `[]`의 두 얼굴 — 리터럴 vs 접근 (헷갈림 근원)
...
```
- **`[]`가 단독으로** → 리스트 생성
- **`[]`가 변수 뒤에** → 꺼내기(인덱스/key). 리스트·dict·문자열 다 `[]`로 접근.

→ 파이썬은 **dict도 `d["key"]`로 `[]` 접근**해서 "[]가 딕셔너리?"처럼 느껴짐. 하지만 그건 *접근 문법*이지 *리터럴(생성)*이 아님.

### 자바 비교 — ⚠️ 자바 `[]`는 "배열", 딕셔너리 아님

| Python | Java |
|---|---|
| `[1,2,3]` 리스트 | 배열 `int[]{1,2,3}` 또는 `List.of(1,2,3)` |
| `(1,2,3)` 튜플 | **없음** (record/배열로 흉내) |
| `{"a":1}` dict | `Map` — `Map.of("a",1)` / `new HashMap<>()` |
| `{1,2,3}` set | `Set` — `Set.of(1,2,3)` / `new HashSet<>()` |

```java
int[] arr = {1, 2, 3};   // 자바: []는 배열 타입, {}는 초기값
arr[0];                  // 배열 인덱스 접근
map.get("key");          // Map은 []로 접근 안 됨! .get() 사용
```

- 자바 `[]` = **배열 전용** (딕셔너리 아님). 자바 Map은 `[]` 인덱싱 불가 → `.get()`/`.put()`.
- 자바 `{}` = 배열 초기화 + 코드블록. 파이썬 `{}` = dict/set 리터럴.
- 자바는 리터럴 문법이 거의 없어 **클래스로 생성**(`new ArrayList<>()`), 파이썬은 `[]`/`{}`로 바로.

### 접근 문법 차이 (핵심)

| | Python | Java |
|---|---|---|
| 리스트/배열 | `lst[0]` | `arr[0]` (같음) |
| dict/map | `d["key"]` | `map.get("key")` (**다름!**) |
